...
export default function CardVacantes({
  ...
  return (
    <>
      <div
        className="border-4 border-neutral-900/80 w-full rounded-3xl shadow-2xl hover:border-yellow-300 ease-in-out duration-300 flex cursor-pointer"
        data-aos="zoom-in"
      ...
      >
        ...
        <div className="py-8 px-8 w-full">
          <h1 className="text-yellow-300 text-2xl md:text-4xl pb-2">
            {titulo}
          </h1>
      ...
      <Modal ref={modalRef}>
        ...
        <p className=" text-neutral-500">{ubicacion}</p>
        <p className=" text-white py-4">{descripcion}</p>
        <a
          href="https://api.whatsapp.com/send/?phone=525510021327&text&type=phone_number&app_absent=0"
          target="_blank"
          className=" w-full"
        >
          <BotonPrincipal text={"Aplicar"} className={"xl:w-full"} />
        </a>
      </Modal>
    </>
  );
