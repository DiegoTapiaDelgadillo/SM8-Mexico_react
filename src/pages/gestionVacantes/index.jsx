...
export default function GestionVacantes() {
  ...
  const { vacantes, createVacante, updateVacante, deleteVacante } = useGestionVacantes();
  ...
  const filtredData = useMemo(() => {
    return vacantes.filter((item) => item.nombre.toLowerCase().includes(search));
  }, [search, vacantes]);
  ...
  return (
    <>
      <div className="bg-neutral-800 h-screen py-20 sm:py-36 px-10 lg:px-20">
        ...
        <p className="text-white text-xs 2xl:text-2xl">Aquí podras gestionar tus vacantes</p>
        ...
        <div className="overflow-y-auto h-3/4">
          <table className="w-full table-auto">
            <thead>
              <tr>
                {["Nombre", "Ubicación", "Descripción", "Editar", "Eliminar"].map((item, index) => (
            ...
            <tbody className="divide-y divide-neutral-500">
              {filtredData.map((item, index) => (
                <tr key={index} className="hover:bg-neutral-900">
                  <td className="text-white text-xs py-4 2xl:text-base">{item.nombre}</td>
                  <td className="text-white text-xs py-4 2xl:text-base">{item.sucursal}</td>
                  <td className="text-white text-xs py-4 2xl:text-base">{item.descripcion}</td>
                  <td className="py-4">
      ...
      <Modal ref={modalRefEdit}>
        ...
        <p className="text-white text-xs md:text-xl">Ingresa los siguientes datos para editar la vacante</p>
        <form onSubmit={handleSumitEdit}>
          ...
          <div className="pt-4">
            <select
              ...
              onChange={(e) => setDataEdit({ ...dataEdit, sucursal: e.target.value })}
          ...
          <div className="pt-4">
            <textarea
              ...
              name="descripcion"
              onChange={(e) => setDataEdit({ ...dataEdit, descripcion: e.target.value })}
            ></textarea>
          </div>
          <div className="pt-4">
      ...
      <Modal ref={modalRefCreate}>
        ...
        <p className="text-white text-xs md:text-xl">Ingresa los siguientes datos para crear la vacante</p>
        <form onSubmit={handleSumitCreate}>
          <div className="pt-2">
            <input
              ...
              name="nombre"
              onChange={(e) => setDataCreate({ ...dataCreate, nombre: e.target.value })}
            />
          </div>
          <div className="pt-4">
            <select
              ...
              id="sucursal"
              className="w-full border rounded-xl p-2 cursor-pointer"
              onChange={(e) => setDataCreate({ ...dataCreate, sucursal: e.target.value })}
          ...
          <div className="pt-4">
            <textarea
              ...
              name="descripcion"
              onChange={(e) => setDataCreate({ ...dataCreate, descripcion: e.target.value })}
            ></textarea>
          </div>
          <div className="pt-4">
